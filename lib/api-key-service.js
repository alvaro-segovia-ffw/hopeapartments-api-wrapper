...
async function createApiKey(input) {
  ...
  if (!params[1]) throw new Error('partnerId is required.');
  if (!params[2]) throw new Error('name is required.');
...
async function updateApiKey(id, input) {
  ...
  if (input.scopes !== undefined) {
    ...
    params.push(JSON.stringify(Array.isArray(input.scopes) ? input.scopes : []));
  }
