...
async function fetchApartmentAddressRecordsLive() {
...
async function fetchApartmentsLive() {
  const config = getOnOfficeConfig();
  const estateRecords = await fetchEstates(config);
  const apartments = estateRecords.map(mapEstateToExport);
  ...
  const picturesRecords =
    estateIds.length > 0 ? await fetchPicturesForEstateIds(config, estateIds) : [];
